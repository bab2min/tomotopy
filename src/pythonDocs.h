...
An instance of this type can be acquired from `tomotopy.LDA.make_doc` method or `tomotopy.LDA.docs` member of each Topic Model instance.)"",
...
DOC_SIGNATURE_EN(LDA_make_doc__doc__,
	"make_doc(self, words)",
	u8R""(Return a new `tomotopy.Document` instance for an unseen document with `words` that can be used for `tomotopy.LDA.infer` method.
...
The returned value is a `list` of `tuple`s with a word and its probability.
...
It can be acquired from `tomotopy.LDA.make_doc` method.
...
DOC_SIGNATURE_EN(DMR_make_doc__doc__,
	...
	u8R""(Return a new `tomotopy.Document` instance for an unseen document with `words` and `metadata` that can be used for `tomotopy.LDA.infer` method.
...
* `tw` : term weighting scheme in `tomotopy.TermWeight`. default is TermWeight.ONE
* `initial_k` : the initial number of topics between 1 ~ 65535.
...
DOC_SIGNATURE_EN(MGLDA___init____doc__,
	"MGLDA(tw=TermWeight.ONE, k_g=1, k_l=1, t=3, alpha_g=0.1, alpha_l=0.1, alpha_mg=0.1, alpha_ml=0.1, eta_g=0.01, eta_l=0.01, gamma=0.1, seed=?)",
...
* `tw` : term weighting scheme in `tomotopy.TermWeight`. default is TermWeight.ONE
* `k_g` : the number of global topics between 1 ~ 65535
* `k_l` : the number of local topics between 1 ~ 65535
