...
''',
    'tomotopy.models': '''`tomotopy.models` 서브모듈은 다양한 토픽 모델 클래스를 제공합니다.
모든 모델은 기본적인 Latent Dirichlet Allocation을 구현하는 `tomotopy.models.LDAModel`을 기반으로 합니다.
파생 모델로는 DMR, GDMR, HDP, MGLDA, PA, HPA, CT, SLDA, LLDA, PLDA, HLDA, DT, PT가 있습니다.
''',
    'tomotopy.viewer': '''..versionadded:: 0.13.0

`tomotopy.viewer`는 tomotopy의 토픽 모델을 웹 브라우저로 시각화하기 위한 모듈입니다.
토픽 모델과 그 결과를 대화형으로 시각화할 수 있는 간단한 방법을 제공합니다.
단, Python 내장 `http.server` 모듈을 사용하기 때문에 프로덕션 웹 서비스에 사용하는 것은 권장되지 않습니다.
...
더 자세한 내용은 `tomotopy.viewer.viewer_server.open_viewer` 함수를 참고하세요.
''',
}
...
> * Yan, F., Xu, N., & Qi, Y. (2009). Parallel inference for latent dirichlet allocation on graphics processing units. In Advances in neural information processing systems (pp. 2134-2142).''',
    ...
    'ProbEstimation.DOCUMENT': '문헌 단위로 확률을 추정합니다.',
    'ProbEstimation.SLIDING_WINDOWS': '슬라이딩 윈도우를 사용하여 확률을 추정합니다.',
    'Segmentation': '\n논문에 제시된 segmentation을 위한 열거형\n',
    'Segmentation.ONE_ONE': '모든 단어 쌍 (w_i, w_j)에 대해 일대일 조합을 생성합니다.',
    'Segmentation.ONE_PRE': '각 단어에 대해 자신 이전의 모든 단어와의 쌍을 생성합니다.',
    'Segmentation.ONE_SUC': '각 단어에 대해 자신 이후의 모든 단어와의 쌍을 생성합니다.',
    'Segmentation.ONE_ALL': '각 단어에 대해 나머지 모든 단어와의 쌍을 생성합니다.',
    'Segmentation.ONE_SET': '각 단어에 대해 전체 단어 집합과의 쌍을 생성합니다.',
    'ConfirmMeasure': '\n논문에 제시된 direct confirm measure를 위한 열거형\n',
    'ConfirmMeasure.DIFFERENCE': '차이(difference) 기반 확인 척도',
    'ConfirmMeasure.RATIO': '비율(ratio) 기반 확인 척도',
    'ConfirmMeasure.LIKELIHOOD': '우도(likelihood) 기반 확인 척도',
    'ConfirmMeasure.LOGLIKELIHOOD': '로그 우도(log-likelihood) 기반 확인 척도',
    'ConfirmMeasure.PMI': '점별 상호 정보량(PMI) 기반 확인 척도',
    'ConfirmMeasure.NPMI': '정규화된 점별 상호 정보량(NPMI) 기반 확인 척도',
    'ConfirmMeasure.LOGCOND': '로그 조건부 확률(log-conditional) 기반 확인 척도',
    'IndirectMeasure': '\n논문에 제시된 indirect confirm measure를 위한 열거형\n',
    'IndirectMeasure.NONE': '간접 확인 척도를 사용하지 않습니다.',
    'IndirectMeasure.COSINE': '코사인 유사도(cosine similarity)를 사용합니다.',
    'IndirectMeasure.DICE': 'Dice 계수를 사용합니다.',
    'IndirectMeasure.JACCARD': 'Jaccard 계수를 사용합니다.',
...
''',
    'CTModel.num_beta_samples': '''beta 파라미터를 표집하는 횟수, 기본값은 10.

CTModel은 각 문헌마다 총 `num_beta_samples` 개수의 beta 파라미터를 표집합니다.
beta 파라미터를 더 많이 표집할 수록, 전체 분포는 정교해지지만 학습 시간이 더 많이 걸립니다.
만약 모형 내에 문헌의 개수가 적은 경우 이 값을 크게하면 더 정확한 결과를 얻을 수 있습니다.
''',
    'CTModel.num_tmn_samples': '''절단된 다변수 정규분포에서 표본을 추출하기 위한 반복 횟수, 기본값은 5.
...
> * Titov, I., & McDonald, R. (2008, April). Modeling online reviews with multi-grain topic models. In Proceedings of the 17th international conference on World Wide Web (pp. 111-120). ACM.''',
    'MGLDAModel.__init__': '''Parameters
----------
tw : Union[int, tomotopy.TermWeight]
    용어 가중치 기법을 나타내는 `tomotopy.TermWeight`의 열거값. 기본값은 TermWeight.ONE 입니다.
min_cf : int
...
transform : Callable[dict, dict]
    ...
    특정한 토픽 모델에 맞춰 임의 키워드 인자를 조작하기 위한 호출가능한 객체
''',
    'PLDAModel.add_doc': '''`labels`를 포함하여 현재 모델에 새 문헌을 추가하고 추가된 문헌의 인덱스를 반환합니다.
...
ignore_empty_words : bool
    `True`일 경우, 빈 `words`에 대해 예외를 발생시키지 않고 None을 반환합니다.
''',
    'PLDAModel.make_doc': '''`words`와 `labels`를 가지고 `tomotopy.models.LDAModel.infer` 메소드에 사용할 수 있는 새 `tomotopy.utils.Document` 인스턴스를 반환합니다.
...
def apply():
    ...
    import tomotopy
    from tomotopy import utils, coherence, label, models, viewer
    ...
    _mod_map = {
        ...
        'tomotopy.models': models,
        'tomotopy.viewer': viewer,
    }
    for path, doc in _MODULE_DOCS.items():
        _mod_map[path].__doc__ = doc

    # Module-level function docs
    _func_map = {
        'load_model': tomotopy.load_model,
        'loads_model': tomotopy.loads_model,
    }
    for key, func in _func_map.items():
        if key in _DOCS:
            func.__doc__ = _DOCS[key]
    ...
    # __pdoc__ overrides for pdoc3 (enum members)
    _tomotopy_pdoc = getattr(tomotopy, '__pdoc__', None)
    if _tomotopy_pdoc is None:
        _tomotopy_pdoc = {}
        tomotopy.__pdoc__ = _tomotopy_pdoc
    _coherence_pdoc = getattr(coherence, '__pdoc__', None)
    if _coherence_pdoc is None:
        _coherence_pdoc = {}
        coherence.__pdoc__ = _coherence_pdoc

    for key, doc in _DOCS.items():
        if '.' not in key:
            continue
        cls_name, attr = key.split('.', 1)
        if cls_name in ('TermWeight', 'ParallelScheme'):
            _tomotopy_pdoc[key] = doc
        elif cls_name in ('ProbEstimation', 'Segmentation', 'ConfirmMeasure', 'IndirectMeasure'):
            _coherence_pdoc[key] = doc
        ...
        if attr == '__init__':
            ...
            if init is not None and hasattr(init, '__doc__'):
                try:
                    ...
                    pass
            continue
        # Try as a descriptor in class __dict__ (property, classmethod, etc.)
        ...
        if obj is not None:
            if hasattr(obj, "__doc__"):
                try:
                    ...
                    pass
            # For classmethod/staticmethod, also set __func__.__doc__
