...
이에 대해서는 `tomotopy.models.LDAModel.save`와 `tomotopy.models.LDAModel.load`에서 더 자세한 내용을 확인할 수 있습니다.
...
`tomotopy`에서 이 두 종류의 문헌을 생성하는 방법은 다릅니다. **모델 안의 문헌**은 `tomotopy.models.LDAModel.add_doc`을 이용하여 생성합니다.
add_doc은 `tomotopy.models.LDAModel.train`을 시작하기 전까지만 사용할 수 있습니다. 
즉 train을 시작한 이후로는 학습 문헌 집합이 고정되기 때문에 add_doc을 이용하여 새로운 문헌을 모델 내에 추가할 수 없습니다.

또한 생성된 문헌의 인스턴스를 얻기 위해서는 다음과 같이 `tomotopy.models.LDAModel.docs`를 사용해야 합니다.
...
**모델 밖의 문헌**은 `tomotopy.models.LDAModel.make_doc`을 이용해 생성합니다. make_doc은 add_doc과 반대로 train을 시작한 이후에 사용할 수 있습니다.
...
새로운 문헌에 대해 추론하기
------------------------------
`tomotopy.models.LDAModel.make_doc`을 이용해 새로운 문헌을 생성했다면 이를 모델에 입력해 주제 분포를 추론하도록 할 수 있습니다. 
새로운 문헌에 대한 추론은 `tomotopy.models.LDAModel.infer`를 사용합니다.
...
infer 메소드는 `tomotopy.utils.Document` 인스턴스 하나를 추론하거나 `tomotopy.utils.Document` 인스턴스의 `list`를 추론하는데 사용할 수 있습니다. 
자세한 것은 `tomotopy.models.LDAModel.infer`을 참조하길 바랍니다.
...
예를 들어 `tomotopy.models.DMRModel`는 `metadata`라는 `str` 타입의 데이터를 요구하고, 
`tomotopy.models.PLDAModel`는 `labels`라는 `List[str]` 타입의 데이터를 요구합니다. 
...
--------------------------------------
0.6.0 버전부터 `tomotopy.models.LDAModel.set_word_prior`라는 메소드가 추가되었습니다. 이 메소드로 특정 단어의 사전분포를 조절할 수 있습니다.
...
* 0.13.0 (2024-08-05)
    * 신규 기능
        ...
        * `tomotopy.models.LDAModel.get_hash()`가 추가되었습니다. 모델의 128bit 해시를 구해줍니다.
        * `ngram_list` 인자가 `tomotopy.utils.SimpleTokenizer`에 추가되었습니다.
    * Bug fixes
        * `Corpus.concat_ngrams` 호출 후에 `spans`이 비일관적인 버그가 수정되었습니다.
        * `tomotopy.models.LDAModel.load()`와 `tomotopy.models.LDAModel.save()`의 병목을 최적화하여 속도를 10배 이상 개선했습니다.
...
* 0.12.6 (2023-12-11)
    * 신규 기능
        * `tomotopy.models.LDAModel.train`과 `tomotopy.models.LDAModel.set_word_prior`에 몇가지 편의 기능을 추가했습니다.
...
* 0.12.4 (2023-01-22)
    ...
    * 버그 수정
        * `tomotopy.utils.Document.get_sub_topic_dist()`가 bad argument 예외를 발생시키는 문제를 해결했습니다.
        * 예외 발생이 종종 크래시를 발생시키는 문제를 해결했습니다.

* 0.12.3 (2022-07-19)
    * 기능 개선
        * 이제 `tomotopy.models.LDAModel.add_doc()`로 빈 문서를 삽입할 경우 예외를 발생시키는 대신 그냥 무시합니다. 새로 추가된 인자인 `ignore_empty_words`를 False로 설정할 경우 이전처럼 예외를 발생시킵니다.
        * 유효하지 않은 토픽들을 모델에서 제거하는 `tomotopy.models.HDPModel.purge_dead_topics()` 메소드가 추가되었습니다.
    * 버그 수정
        * `tomotopy.models.SLDAModel`에서 `nuSq` 값을 지정할 때 발생하는 문제를 해결했습니다. (by @jucendrero)
        * `tomotopy.coherence.Coherence`가 `tomotopy.models.DTModel`에 대해서 작동하지 않는 문제를 해결했습니다.
        * `train()` 호출 전에 `make_dic()`을 호출할 때 종종 크래시가 발생하는 문제를 해결했습니다.
        * seed가 고정된 상태에서도 `tomotopy.models.DMRModel`, `tomotopy.models.GDMRModel`의 결과가 다르게 나오는 문제를 해결했습니다.
        * `tomotopy.models.DMRModel`, `tomotopy.models.GDMRModel`의 파라미터 최적화 과정이 부정확하던 문제를 해결했습니다.
        * `tomotopy.models.PTModel.copy()` 호출 시 종종 크래시가 발생하는 문제를 해결했습니다.

* 0.12.2 (2021-09-06)
    * `min_cf > 0`, `min_df > 0`나 `rm_top > 0`로 설정된 `tomotopy.models.HDPModel`에서 `convert_to_lda`를 호출할때 크래시가 발생하는 문제를 해결했습니다.
    * `tomotopy.utils.Document.get_topics`와 `tomotopy.utils.Document.get_topic_dist`에 `from_pseudo_doc` 인자가 추가되었습니다. 
      이 인자는 `PTModel`에 대해서만 유효하며, 이를 통해 토픽 분포를 구할 때 가상 문헌을 사용할지 여부를 선택할 수 있습니다.
    * `tomotopy.models.PTModel` 생성시 기본 인자값이 변경되었습니다. `p`를 생략시 `k * 10`으로 설정됩니다.
    * `make_doc`으로 생성한 문헌을 `infer` 없이 사용할 경우 발생하는 크래시를 해결하고 경고 메세지를 추가했습니다.
    * 내부 C++코드가 clang c++17 환경에서 컴파일에 실패하는 문제를 해결했습니다.

* 0.12.1 (2021-06-20)
    * `tomotopy.models.LDAModel.set_word_prior()`가 크래시를 발생시키던 문제를 해결했습니다.
    * 이제 `tomotopy.models.LDAModel.perplexity`와 `tomotopy.models.LDAModel.ll_per_word`가 TermWeight가 ONE이 아닌 경우에도 정확한 값을 반환합니다.
    * 용어가중치가 적용된 빈도수를 반환하는 `tomotopy.models.LDAModel.used_vocab_weighted_freq`가 추가되었습니다.
    * 이제 `tomotopy.models.LDAModel.summary()`가 단어의 엔트로피뿐만 아니라, 용어 가중치가 적용된 단어의 엔트로피도 함께 보여줍니다.

* 0.12.0 (2021-04-26)
    * 이제 `tomotopy.models.DMRModel`와 `tomotopy.models.GDMRModel`가 다중 메타데이터를 지원합니다. (https://github.com/bab2min/tomotopy/blob/main/examples/dmr_multi_label.py 참조)
    * `tomotopy.models.GDMRModel`의 성능이 개선되었습니다.
...
* 0.11.0 (2021-03-26) (삭제됨)
    * 짧은 텍스트를 위한 토픽 모델인 `tomotopy.models.PTModel`가 추가되었습니다.
    * `tomotopy.models.HDPModel.infer`가 종종 segmentation fault를 발생시키는 문제가 해결되었습니다.
    ...
    * `tomotopy.models.DMRModel.lambdas`와 `tomotopy.models.DMRModel.alpha`가 잘못된 값을 제공하던 문제가 해결되었습니다.
    * `tomotopy.models.GDMRModel`에 범주형 메타데이터 지원이 추가되었습니다. (https://github.com/bab2min/tomotopy/blob/main/examples/gdmr_both_categorical_and_numerical.py 참조)
    * Python3.5 지원이 종료되었습니다.

* 0.10.2 (2021-02-16)
    * `tomotopy.models.CTModel.train`가 큰 K값에 대해 실패하는 문제가 수정되었습니다.
    * `tomotopy.utils.Corpus`가 `uid`값을 잃는 문제가 수정되었습니다.

* 0.10.1 (2021-02-14)
    * `tomotopy.utils.Corpus.extract_ngrams`에 빈 문헌을 입력시 발생하던 에러를 수정했습니다.
    * `tomotopy.models.LDAModel.infer`가 올바른 입력에도 예외를 발생시키던 문제를 수정했습니다.
    * `tomotopy.models.HLDAModel.infer`가 잘못된 `tomotopy.utils.Document.paths` 값을 생성하는 문제를 수정했습니다.
    * `tomotopy.models.HLDAModel.train`에 새로운 파라미터 `freeze_topics`가 추가되었습니다. 이를 통해 학습 시 신규 토픽 생성 여부를 조정할 수 있습니다.

* 0.10.0 (2020-12-19)
    * `tomotopy.utils.Corpus`와 `tomotopy.models.LDAModel.docs` 간의 인터페이스가 통일되었습니다. 이제 동일한 방법으로 코퍼스 내의 문헌들에 접근할 수 있습니다.
    ...
    * `tomotopy.models.LDAModel.add_corpus`가 추가되었고, `tomotopy.models.LDAModel.infer`가 Raw 코퍼스를 입력으로 받을 수 있게 되었습니다.
    ...
    * `tomotopy.models.HDPModel` 학습 시 종종 NaN이 발생하는 문제를 해결했습니다.
    * 이제 Python3.9를 지원합니다.
    * py-cpuinfo에 대한 의존성이 제거되고, 모듈 로딩속도가 개선되었습니다.
    
* 0.9.1 (2020-08-08)
    * 0.9.0 버전의 메모리 누수 문제가 해결되었습니다.
    * `tomotopy.models.CTModel.summary()`가 잘못된 결과를 출력하는 문제가 해결되었습니다.

* 0.9.0 (2020-08-04)
    * 모델의 상태를 알아보기 쉽게 출력해주는 `tomotopy.models.LDAModel.summary()` 메소드가 추가되었습니다.
    * 난수 생성기를 [EigenRand]로 대체하여 생성 속도를 높이고 플랫폼 간의 결과 차이를 해소하였습니다.
    * 이로 인해 `seed`가 동일해도 모델 학습 결과가 0.9.0 이전 버전과 달라질 수 있습니다.
    * `tomotopy.models.HDPModel`에서 간헐적으로 발생하는 학습 오류를 수정했습니다.
    * 이제 `tomotopy.models.DMRModel.alpha`가 메타데이터별 토픽 분포의 사전 파라미터를 보여줍니다.
    * `tomotopy.models.DTModel.get_count_by_topics()`가 2차원 `ndarray`를 반환하도록 수정되었습니다.
    * `tomotopy.models.DTModel.alpha`가 `tomotopy.models.DTModel.get_alpha()`와 동일한 값을 반환하도록 수정되었습니다.
    * `tomotopy.models.GDMRModel`의 document에 대해 `metadata` 값을 얻어올 수 없던 문제가 해결되었습니다.
    * 이제 `tomotopy.models.HLDAModel.alpha`가 문헌별 계층 분포의 사전 파라미터를 보여줍니다.
    * `tomotopy.models.LDAModel.global_step`이 추가되었습니다.
    * 이제 `tomotopy.models.MGLDAModel.get_count_by_topics()`가 전역 토픽과 지역 토픽 모두의 단어 개수를 보여줍니다.
    * `tomotopy.models.PAModel.alpha`, `tomotopy.models.PAModel.subalpha`, `tomotopy.models.PAModel.get_count_by_super_topic()`이 추가되었습니다.

[EigenRand]: https://github.com/bab2min/EigenRand

* 0.8.2 (2020-07-14)
    * `tomotopy.models.DTModel.num_timepoints`와 `tomotopy.models.DTModel.num_docs_by_timepoint` 프로퍼티가 추가되었습니다.
    * `seed`가 동일해도 플랫폼이 다르면 다른 결과를 내던 문제가 일부 해결되었습니다. 이로 인해 32bit 버전의 모델 학습 결과가 이전 버전과는 달라졌습니다.

* 0.8.1 (2020-06-08)
    * `tomotopy.models.LDAModel.used_vocabs`가 잘못된 값을 반환하는 버그가 수정되었습니다.
    * 이제 `tomotopy.models.CTModel.prior_cov`가 `[k, k]` 모양의 공분산 행렬을 반환합니다.
    * 이제 인자 없이 `tomotopy.models.CTModel.get_correlations`를 호출하면 `[k, k]` 모양의 상관관계 행렬을 반환합니다.

* 0.8.0 (2020-06-06)
    * NumPy가 tomotopy에 도입됨에 따라 많은 메소드와 프로퍼티들이 `list`가 아니라 `numpy.ndarray`를 반환하도록 변경되었습니다.
    * Tomotopy에 새 의존관계 `NumPy >= 1.10.0`가 추가되었습니다..
    * `tomotopy.models.HDPModel.infer`가 잘못된 추론을 하던 문제가 수정되었습니다.
    * HDP 모델을 LDA 모델로 변환하는 메소드가 추가되었습니다.
    * `tomotopy.models.LDAModel.used_vocabs`, `tomotopy.models.LDAModel.used_vocab_freq`, `tomotopy.models.LDAModel.used_vocab_df` 등의 새로운 프로퍼티가 모델에 추가되었습니다.
    * 새로운 토픽 모델인 g-DMR(`tomotopy.models.GDMRModel`)가 추가되었습니다.
...
* 0.7.1 (2020-05-08)
    * `tomotopy.models.HLDAModel`용으로 `tomotopy.utils.Document.paths`가 새로 추가되었습니다.
    * `tomotopy.label.PMIExtractor` 사용시에 발생하던 메모리 문제가 해결되었습니다.
    * gcc 7에서 발생하던 컴파일 오류가 해결되었습니다.

* 0.7.0 (2020-04-18)
    * `tomotopy.models.DTModel`이 추가되었습니다.
    * `tomotopy.utils.Corpus.save`가 제대로 작동하지 않는 버그가 수정되었습니다.
    * `tomotopy.utils.Document.get_count_vector`가 추가되었습니다.
...
* 0.6.0 (2020-03-22) (삭제됨)
    ...
    * 어휘-주제 분포의 사전 확률을 조절할 수 있는 `tomotopy.models.LDAModel.set_word_prior` 메소드가 추가되었습니다.
...
* 0.5.2 (2020-03-01)
    * `tomotopy.models.LLDAModel.add_doc` 실행시 segmentation fault가 발생하는 문제를 해결했습니다.
    * `tomotopy.models.HDPModel`에서 `infer` 실행시 종종 프로그램이 종료되는 문제를 해결했습니다.
    * `tomotopy.models.LDAModel.infer`에서 ps=tomotopy.ParallelScheme.PARTITION, together=True로 실행시 발생하는 오류를 해결했습니다.

* 0.5.1 (2020-01-11)
    * `tomotopy.models.SLDAModel.make_doc`에서 결측값을 지원하지 않던 문제를 해결했습니다.
    * `tomotopy.models.SLDAModel`이 이제 결측값을 지원합니다. 결측값을 가진 문헌은 토픽 모델링에는 참여하지만, 응답 변수 회귀에서는 제외됩니다.

* 0.5.0 (2019-12-30)
    * `tomotopy.models.PAModel.infer`가 topic distribution과 sub-topic distribution을 동시에 반환합니다.
    * `tomotopy.utils.Document`에 get_sub_topics, get_sub_topic_dist 메소드가 추가되었습니다. (PAModel 전용)
    * `tomotopy.models.LDAModel.train` 및 `tomotopy.models.LDAModel.infer` 메소드에 parallel 옵션이 추가되었습니다. 이를 통해 학습 및 추론시 사용할 병렬화 알고리즘을 선택할 수 있습니다.
...
* 0.4.2 (2019-11-30)
    * `tomotopy.models.LLDAModel`와 `tomotopy.models.PLDAModel` 모델에서 토픽 할당이 잘못 일어나던 문제를 해결했습니다.
    * `tomotopy.utils.Document` 및 `tomotopy.Dictionary` 클래스에 가독성이 좋은 __repr__가 추가되었습니다.

* 0.4.1 (2019-11-27)
    * `tomotopy.models.PLDAModel` 생성자의 버그를 수정했습니다.

* 0.4.0 (2019-11-18)
    * `tomotopy.models.PLDAModel`와 `tomotopy.models.HLDAModel` 토픽 모델이 새로 추가되었습니다.

* 0.3.1 (2019-11-05)
    * `min_cf` 혹은 `rm_top`가 설정되었을 때 `get_topic_dist()`의 반환값이 부정확한 문제를 수정하였습니다.
    * `tomotopy.models.MGLDAModel` 모델의 문헌의 `get_topic_dist()`가 지역 토픽에 대한 분포도 함께 반환하도록 수정하였습니다..
    * `tw=ONE`일때의 학습 속도가 개선되었습니다.

* 0.3.0 (2019-10-06)
    * `tomotopy.models.LLDAModel` 토픽 모델이 새로 추가되었습니다.
...
* 0.2.0 (2019-08-18)
    * `tomotopy.models.CTModel`와 `tomotopy.models.SLDAModel` 토픽 모델이 새로 추가되었습니다.
...
* 0.1.4 (2019-08-05)
    ...
    * `tomotopy.models.PAModel.get_topic_words`가 하위토픽의 단어 분포를 제대로 반환하지 못하는 문제를 해결하였습니다.
